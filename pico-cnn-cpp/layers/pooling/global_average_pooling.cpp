        ...
        for (uint32_t channel = 0; channel < num_channels; channel++) {
            ...
            for (uint32_t row = 0; row < height; row++) {
                for (uint32_t col = 0; col < width; col++) {
                    if (num_dims == 4) {
                        global_sum += input->access(batch, channel, row, col);
                    } else if (num_dims == 3) {
                        global_sum += input->access(batch, channel, col);
                    }
                }
            }

            if (num_dims == 4) {
                output->access(batch, channel, 0, 0) = global_sum / (fp_t)(height*width);
            } else if (num_dims == 3) {
                output->access(batch, channel, 0) = global_sum / (fp_t)(height*width);
            }

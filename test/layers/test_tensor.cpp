...
//    shape6 = new pico_cnn::naive::TensorShape(10);
    ...
    tensor1 = new pico_cnn::naive::Tensor(4, 5, 5);
    tensor2 = new pico_cnn::naive::Tensor(1, 3, 20, 20);
    tensor3 = new pico_cnn::naive::Tensor(1, 3, 20, 20);
...
void TestTensor::runTestTensorAccess() {
    ...
    for(int channel = 0; channel < 4; channel++) {
        for (int row = 0; row < 5; ++row) {
            for (int col = 0; col < 5; ++col) {
                tensor1->access(channel, row, col, 5, 5) = cnt;
                cnt+=1.0;
            }
        }
    }
    ...
    for(int channel = 0; channel < 4; channel++) {
        for (int row = 0; row < 5; ++row) {
            for (int col = 0; col < 5; ++col) {
                CPPUNIT_ASSERT(abs(tensor1->access(channel, row, col, 5, 5) - cnt) < 0.0001);
                cnt+=1.0;
            }
        }
    }

    fp_t tmp = tensor1->access(0,0,0, 5, 5);
    tmp += 10.0;
    CPPUNIT_ASSERT(abs(tmp - tensor1->access(0, 0, 0, 5, 5)) == 10);
        ...
        for (int channel = 0; channel < 3; channel++) {
            for (int row = 0; row < 20; row++) {
                for (int col = 0; col < 20; col++) {
                    tensor2->access(batch, channel, row, col, 3, 20, 20) = cnt;
                    cnt += 1.0;
                }
            }
        }
    ...
    for(int batch = 0; batch < 1; batch++) {
        for (int channel = 0; channel < 3; channel++) {
            for (int row = 0; row < 20; ++row) {
                for (int col = 0; col < 20; ++col) {
                    CPPUNIT_ASSERT(abs(tensor2->access(batch, channel, row, col, 3, 20, 20) - cnt) < 0.0001);
                    cnt += 1.0;
                }
            }
        }
    ...
    for(int batch = 0; batch < 1; batch++) {
        for (int channel = 0; channel < 3; channel++) {
            for (int row = 0; row < 20; row++) {
                for (int col = 0; col < 20; col++) {
                    tensor3->access(batch, channel, row, col, 3, 20, 20) = cnt;
                    cnt += 1.0;
                }
            }
        }
...
void TestTensor::runTestTensorAddition() {
    ...
    auto *input_tensor1 = new pico_cnn::naive::Tensor(3, 4);
    auto *input_tensor2 = new pico_cnn::naive::Tensor(3, 4);
    auto *expected_output_tensor = new pico_cnn::naive::Tensor(3, 4);
...
void TestTensor::runTestTensorGetPtr() {
    for(int batch = 0; batch < 1; batch++) {
        for (int channel = 0; channel < 3; channel++) {
            ...
            for (int row = 0; row < 20; row++) {
                for (int col = 0; col < 20; col++) {
                    tmp1 = tensor2->access(batch, channel, row, col, 3, 20, 20);
...
void TestTensor::runTestTensorExpandPadding() {
    ...
    auto *orig_tensor = new pico_cnn::naive::Tensor(1, 3, 2, 2);
    auto *expected_extended_tensor = new pico_cnn::naive::Tensor(1, 3, 4, 4);
    ...
    auto *orig_tensor_1d = new pico_cnn::naive::Tensor(1, 3, 4);
    auto *expected_extended_tensor_1d = new pico_cnn::naive::Tensor(1, 3, 8);
    ...
    auto *orig_tensor_2d = new pico_cnn::naive::Tensor(7, 5);
    auto *expected_extended_tensor_2d = new pico_cnn::naive::Tensor(12, 7);
...
void TestTensor::runTestTensorConcatDim0() {
    auto *input_tensor1 = new pico_cnn::naive::Tensor(1, 2, 3, 3);
    auto *input_tensor2 = new pico_cnn::naive::Tensor(1, 3, 3, 3);
    auto *expected_output_tensor = new pico_cnn::naive::Tensor(1, 5, 3, 3);
    auto *output_tensor = new pico_cnn::naive::Tensor(1, 5, 3, 3);

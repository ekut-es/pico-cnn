    ...
    // concatenate along channel rows
        ...
        for(input_channel = 0; input_channel < num_input_channels; input_channel++) {
            for(channel_row = 0; channel_row < channel_height; channel_row++) {
                for(input_id = 0; input_id < num_input_channels; input_id++) {
                    memcpy(&output_channels[input_channel][input_id*channel_width],
                     &inputs[input_id][input_channel][input_id*channel_width],
                     channel_width);
                }
            }
